...
- Room 2.7.2 (`room-runtime`, `room-ktx`, KSP `room-compiler`) for books, progress, highlights + vocabulary (v7 — SM-2 vocabulary + scan dedup hash/path)
...
│       │   │   ├── db/ { FolioDatabase v7, dao/{BookDao, ReadingProgressDao, HighlightDao, VocabularyDao}, entity/{BookEntity (fileHash/importedFromPath), ReadingProgress, Highlight (anchorText), VocabularyCard (SM-2)} }
...
│       │       │   └── components/ { ReadingProgressBar.kt, HighlightOverlay.kt (pressure/tilt Multiply + lasso), XRayBottomSheet.kt, DictionaryPopup.kt, ExpandableDiagram.kt } # bounding-box image expansion (tap to strip white margins, cached, full-width)
│       │       └── vocabulary/
...
- **Progress (§6)** — `FolioDatabase` (`Room` v7) with `ReadingProgress` (`bookId` PK, `chapterIndex`, `paragraphIndex`, `lastReadMillis`). `ReadingViewModel` observes/saves position via `ReadingProgressDao`; restored on next open.
